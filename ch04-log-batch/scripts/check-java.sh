...
# src/ 아래의 자바 파일이 수정됐으면 AGENTS.md의 스프링 규약과 테스트 규약을 검사하고
# 곧바로 컴파일해서 오류를 에이전트에게 돌려준다.
...
# 생성자가 하나면 @Autowired를 생략할 수 있고 이 프로젝트는 생성자 주입만 쓰므로
# 운영 코드에는 두 애너테이션이 필요 없다. 발견되면 규약 위반으로 알려 준다.
# 테스트 클래스는 AGENTS.md에서 예외로 두었으므로 src/main 아래만 검사한다.
case "$file_path" in
  src/main/*.java|*/src/main/*.java)
    if [ -f "$file_path" ] && grep -Eq '@(Autowired|Inject)\b' "$file_path"; then
      echo "규약 위반: ${file_path}에 @Autowired 또는 @Inject가 있다." >&2
      echo "AGENTS.md의 Spring 규약에 따라 의존 객체는 생성자 주입으로 받아라. 생성자가 하나면 애너테이션 없이 주입된다." >&2
      exit 2
    fi
    ;;
esac
